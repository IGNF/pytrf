...
def write_res(snx, acs, w, d, fres, fyml):
    ...
    # Open output files
    with open(fres, 'w') as fres, open(fyml, 'w') as fyml:
...
def write_sum(dacs, w, opt, nsta, ndat, Tdat, wdat, ncore, Tcore, wcore, dxpo, dypo, dxpor, dypor, dlod, nolog, missing, fsum, fyml):
    ...
    # Open output files
    with open(fsum, 'w') as fsum, open(fyml, 'w') as fyml:
...
def write_stacksum(w, opt, inputs, combsnx, nobs, wrms, fsum):
    ...
    with open(fsum, 'w') as f:
        ...
        print(' whose validity period is given in {0}.'.format(opt.stackdsc), file=f)
        print('', file=f)
        ...
        print(' models are given in {0}.'.format(opt.stackpsd), file=f)
